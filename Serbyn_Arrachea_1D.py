...
from Superconductor_1D_without_SOC import get_sigma, Rho_k, Fermi_function_derivative, get_sigma_quad, get_sigma_k,get_sigma_quad_k, integrand
...
fig, ax = plt.subplots()
ax.plot(omega, [np.trace(integrand(k, omega, w_0, Gamma, B_x, B_y, Delta, mu, beta)) for omega in omega])
ax.plot(epsilon_n, [integrand_Serbyn(k, epsilon_n, w_0, Gamma, Delta, mu) for epsilon_n in epsilon_n])
